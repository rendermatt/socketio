...
const format_msg = module.exports.format_msg = msg => msg.replace("\\\\", "\f") // temp rm \\
  ...
  .replace(/f\s*u\s*c\s*k/ig, "truck")
  .replace(/s\s*h\s*[1i]\s*t/ig, "ship")
  .replace(/b\s*[1i]\s*t\s*c\s*h/ig, "female dog")
  .replace(/s\s*h\s*u\s*t\s*u\s*p/ig, "shut down")
  .replace(/t\s*r\s*a\s*n\s*n\s*y/ig, "tyrannosaurus rex")
  .replace(/d\s*[1i]\s*k\s*e/ig, "chuiwawa")
  .replace(/f\s*a\s*g\s*g\s*o\s*t/ig, "french fry")
  .replace(/n\s*[1i]\s*g\s*g\s*(e|a)\s*r?/ig, "nacho")
  .replace(/t\s*r\s*u\s*m\s*p/ig, "trombone") // joke
  .replace(/J\s*o\s*e\s*B\s*[1i]\s*d\s*e\s*n/ig, "Jeffery Bezos") // joke
  .replace(/h\s*e\s*f\s*f\s*e\s*r/ig, "helper")
  .replace(/s\s*l\s*u\s*t/ig, "serial killer")
  .replace(/d\s*[1i]\s*c\s*k/ig, "dinosaur")
  .replace(/c\s*o\s*c\s*k/ig, "cabbage")
  .replace(/c\s*a\s*b\s*l\s*e/ig, "cock") // joke
  .replace(/c\s*u\s*n\s*t/ig, "putter")
  .replace(/p\s*u\s*s\s*s\s*y/ig, "kitty")
  .replace(/p\s*e\s*n\s*i\s*s/ig, "pencil")
  .replace(/v\s*a\s*g\s*[1i]\s*n\s*a/ig, "vinegar")
