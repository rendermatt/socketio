...
const format_msg = module.exports.format_msg = msg => msg.replace("\\\\", "\f") // temp rm \\
  ...
  .replace(/s\s*h\s*[1li]\s*t/ig, "ship")
  .replace(/b\s*[1li]\s*t\s*c\s*h/ig, "female dog")
  .replace(/s\s*h\s*u\s*t\s*u\s*p/ig, "shut down")
  .replace(/t\s*r\s*a\s*n\s*n\s*y/ig, "tyrannosaurus rex")
  .replace(/d\s*[1li]\s*k\s*e/ig, "chuiwawa")
  .replace(/f\s*a\s*g\s*g\s*o\s*t/ig, "french fry")
  .replace(/n\s*[1li]\s*g\s*g\s*(e|a)\s*r?/ig, "nacho")
  .replace(/j\s*o\s*s\s*e/ig, "jesus")
  .replace(/t\s*r\s*u\s*m\s*p/ig, "trombone") // joke
  .replace(/J\s*o\s*e\s*B\s*[1li]\s*d\s*e\s*n/ig, "Jeffery Bezos") // joke
  .replace(/h\s*e\s*f\s*f\s*e\s*r/ig, "helper")
  .replace(/s\s*l\s*u\s*t/ig, "serial killer")
  .replace(/d\s*[1li]\s*c\s*k/ig, "dinosaur")
